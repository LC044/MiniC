...
/* 修改 */
// 函数定义指令
FuncDefIRInst::FuncDefIRInst(Value *func_name, Value *paras, ValueType ret_type = ValueType::ValueType_Int) :
    IRInst(IRINST_OP_VAR_DEF, _ret_type)
{
    srcValues.push_back(_func_name);
...
{
    // todo 暂时只考虑int型，以后可以继续添加
    Value *ret_type = dstValue;
    std::string type;
    if (ret_type->type == ValueType::ValueType_Int) {
        type = "i32";
    }
    str = "define " + type + "@" + ret_type->getName();
