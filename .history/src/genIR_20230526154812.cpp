...
static bool ir_def_array(struct ast_node *node)
{
    ...
    ir_def_array_dims(temp_dims, val);
    for (int i = 0; i < 10;i++) {
        printf("%d ", val->dims[i]);
    }
    // 第二个孩子是数组维度节点
...
// 获取数组维度
static bool ir_def_array_dims(struct ast_node *node, Value *val)
{
    // 第一个孩子是当前维度的值
    struct ast_node *temp_dim = node->sons[0];
    val->dims[0] = temp_dim->attr.integer_val;
    // 第二个孩子节点是下一个维度的值
    struct ast_node *next_dims = node->sons[1];
    int i = 1;
    while (next_dims) {
        val->dims[i] = next_dims->sons[0]->attr.integer_val;
        next_dims = next_dims->sons[1];
    }
    return true;
}
// 产生IR显示的IR指令
