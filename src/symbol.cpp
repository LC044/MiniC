...
void SymbolTable::addFunction(std::string func_name, FuncSymbol *symbol)
{
    ...
    varsMap.emplace(func_name, symbol);
}
/// @brief 新建变量型Value
/// @param name 变量ID
/// @param type 变量类型
void newVarValue(std::string name, ValueType type)
{
    ...
    if (pIter != symbolTable->varsMap.end()) {
        ...
        symbolTable->varsMap.emplace(name, temp);
    }
}
...
Value *newConstValue(int intVal)
{
    ...
    symbolTable->varsMap.emplace(temp->name, temp);
...
{
    auto pIter1 = symbolTable->funcsMap.find(func_name);
...
FuncSymbol *findFuncValue(std::string name)
{
    ...
    auto pIter = symbolTable->funcsMap.find(name);
...
bool GlobalIsExist(std::string name)
{
    auto pIter = symbolTable->varsMap.find(name);
    if (pIter == symbolTable->varsMap.end()) {
        auto pIter1 = symbolTable->funcsMap.find(name);
        if (pIter1 == symbolTable->funcsMap.end()) {
            return false;
        } else {
            return true;
        }
...
{
    // 清理Hash表中的所有创建的Value
    for (auto [k, v] : symbolTable->varsMap) {
        delete v;
    }
    // Hash表清空
    symbolTable->varsMap.clear();
}
